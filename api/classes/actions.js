...
class Actions {
  ...
  async buildTemplatesAndEmail(bounty_id =null){
    ...
    for(var entry of projectFiles){
      await this.box.moveFile(entry.id, bountyFolderId)
    }
...
isOdd(n) {
  ...
  async deploykeyword(){
   ...
   var brand_id = await mongoose.connection.db
    ...
    setTimeout( async (keyword, bounty_folder_id) => {
    ...
    }, 15000, this.req.body.keyword, brand_id["bountyFolderId"]);
    ...
    */
    ...
    async getnextinprogressfilmingbounty(minId ="61f978d69c507e6abd5dc9ef"){
      ...
      var aggregate = [
        ...
        output["script_id"] = result.bountyScript;
    ...
    async getscriptid(){
        ...
        output["script_text"] = fileText;
        output["script_id"] = googleScriptId;
 ...
 */
 ...
 async getselectedscripts(){

    var selectedBounties = await mongoose.connection.db
        .collection("bounties")
        .find({created_by: mongoose.Types.ObjectId(this.user._id), selected: true }, {projection: { bountyScript: 1, _id:0 }}).toArray();

    var bountyDocs = []
    for(var item of selectedBounties){
      bountyDocs.push(item.bountyScript)
    }
