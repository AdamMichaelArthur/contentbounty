...
var datasource_file = './api/classes/datasources.json'
...
"key", "owner","getkeys","selected","nextselected","prevselected", "mergefields","emailfields", "count", "bulk"]
...
async function del(req, res, next, model){
	...
	var id = helpers.getParameter(req.params[0], "id");
	var doc = await model.deleteOne({ _id:id });
	console.log(152, doc);
...
async function calculateSubsequentAggregationPages(aggregate, limit, modelName, model, req, res, firstId, lastId, page){
	...
	console.log(2118, page, lastId);
  	...
  	//console.log(2116, util.inspect(aggregate, false, null, true /* enable colors */))
...
function getWithSort(req, res, next, model)
{
	...
	if(all)
		account = "all"
	console.log(req.params[0], account)
	if(typeof max_records == 'undefined')
		max_records = 10;
	else
		max_records = parseInt(max_records);
	console.log(843)
	var filter = getFilter(req)
	console.log(845, filter)
	var sort = getSort(req);
	console.log(847, sort)
...
async function selectall(req, res, next, model){

	// We need to mandate that a filter be sent here...otherwise
	// a shedton of documents can get selected
